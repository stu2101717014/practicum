...
var requestURL = 'http://localhost:17519';
var queryParams = {
  "IsCute": false,
  "IsSmart": false,
  "IsEducated": false,
  "UseCheckBoxesForFilteringData": false,
  "DogBreed": "All"
};
var receivedData = [];
...
function displaySmallGrid() {
  ...
  $('#form-dogs').remove();
  var container = $('<div class="container" id="dogs-grid-small-icons-container"></div>');
  var table = $('<table class="table mb-4"></table>');
  ...
  for (var i = 0; i < receivedData.length; i++) {
    ...
    for (var _i = 0, _Object$entries = Object.entries(receivedData[i]); _i < _Object$entries.length; _i++) {
...
function displayDogsLargeGrid() {
  ...
  $('#dogs-grid-small-icons-container').remove();
  $('#form-dogs').remove();
  var container = $('<div className="container" id="dogs-grid-large-icons-container"className="form-request-stage-two"></div>');

  for (var i = 0; i < receivedData.length; i += 3) {
    var row = $('    <div className="row">\n </div>');

    for (var j = 0; j < 3 && i + j < receivedData.length; j++) {
      var cell = $('     <div class="card" style="width: 18rem;">\n' + '       <img src="data:image/png;base64,' + receivedData[i + j]['image'] + '" class="card-img-top form-request-stage-two" alt="...">\n' + '       <div class="card-body form-request-stage-two">\n' + '         <p class="card-text form-request-stage-two">' + 'Is cute :' + ' ' + receivedData[i + j]['isCute'] + '<br>' + 'Is smart :' + ' ' + receivedData[i + j]['isSmart'] + '<br>' + 'Is educated :' + ' ' + receivedData[i + j]['isEducated'] + '<br>' + '         </p>\n' + '       </div>\n' + '     </div>\n');
      $(row).append(cell);
    }
...
function checkBoxSetDogProp(ev) {
  var param = ev.target.id;
  var value = $('#' + ev.target.id).is(':checked');
...
function changeClassesOfClickedButton() {
  var classes = $('#dog-breed-drop-down-button').attr("class");
...
function formDataReceived(result) {
  var form = $('#form-dogs')[0];
  var formUndefined = false;
  ...
  for (var el in result) {
    if (result[el].item1 === 'DogBreed') {
      $(dropdown).find('#drop-down-ul-dog-breed').append('<li style="float:left"><a class="dropdown-item" href="#" + id="' + result[el].item2 + '">' + result[el].item2 + '</a></li>');
  ...
  for (var _el in result) {
    if (result[_el].item1 !== 'DogBreed') {
      var _divEl = $('<div class="mb-3">\n' + '    <label class="form-label">' + result[_el].item1 + '</label>\n' + '    <input type="checkbox" class="form-check-input" checked="' + queryParams[result[_el].item2] + '" id="' + result[_el].item1 + '" >\n' + '  </div>\n');

      $(_divEl).find('#' + result[_el].item1).click(checkBoxSetDogProp);
      $(_divEl).find('#' + result[_el].item1).prop('checked', queryParams[result[_el].item1]);
      $(form).prepend(_divEl);
    }
  }

  var divEl = $('<div class="mb-3">\n' + '    <label class="form-label">' + 'UseCheckBoxesForFilteringData' + '</label>\n' + '    <input type="checkbox" class="form-check-input"  id="' + 'UseCheckBoxesForFilteringData' + '" >\n' + '  </div>\n');
  ...
  if (formUndefined) {
    var dogSubmitBtn = $('<button type="submit" class="btn btn-primary" id="submit-form-button" >Submit</button>');
